...
export async function getContent(params?: { status?: string; campaign_id?: string }): Promise<ContentAsset[]> {
...
export async function scheduleContent(id: string, scheduled_for: string): Promise<ContentAsset> {
  return piFetch(`/brand/content/${id}/schedule`, {
    method: "POST",
    body: JSON.stringify({ scheduledFor: scheduled_for }),
  });
}

export async function getLeads(params?: { status?: string }): Promise<RecruitmentLead[]> {
  const qs = params ? `?${new URLSearchParams(params as Record<string, string>).toString()}` : "";
  return piFetch(`/brand/leads${qs}`);
}

export async function createLead(data: Omit<RecruitmentLead, "id" | "created_at">): Promise<RecruitmentLead> {
...
export async function updateLead(id: string, data: Partial<RecruitmentLead>): Promise<RecruitmentLead> {
